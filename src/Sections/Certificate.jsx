...
const CertificateCard = ({ certificate, onClick }) => (
  ...
  >
    ...
    <div className="flex items-center justify-center mb-4"> {/* Changed to center the icon */}
      {certificate.icon}
    </div>
    <h3 className="text-xl font-semibold text-blue-600 text-center">
      {certificate.title} {/* Centered the title */}
    </h3>
    <p className="mt-2 text-gray-600 text-center">Issuer: {certificate.issuer}</p> {/* Centered the text */}
    <p className="text-gray-600 text-center">Date: {certificate.date}</p> {/* Centered the text */}
    <p className="mt-2 text-gray-500 text-sm text-center">{certificate.description}</p> {/* Centered the text */}
    <motion.div
    ...
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <motion.h1
          ...
          transition={{ duration: 0.5 }}
        >
          <span className="text-black inline-block mr-2 px-2 py-1">My</span>
          Certificates
        </motion.h1>
        {selectedCertificate ? (
          <div>
